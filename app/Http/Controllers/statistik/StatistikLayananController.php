...
class StatistikLayananController extends Controller
{
    ...
    public function export_excel(Request $request)
    {
        ...
        $data = Jadwal::leftjoin('layanan','jadwal.layanan_id','=','layanan.id')
                        ...
                        ->where('jadwal.tanggal','<=', $request->until = Carbon::parse($request->until))
                        ->where('status.nama','=','Selesai')
                        ->select('jadwal.tanggal as tanggal','layanan.nama as nama','layanan.harga as harga','a.name as psikolog','b.name as klien')  
                        ->orderBy('layanan.nama')
                        ->orderBy('tanggal','asc')
                        ->get()
                        ->toArray();
        ...
        foreach($data as $i => $layanan){
            $collection[$i] = [
                'nama' => $layanan['nama'],
                'tanggal' => $layanan['tanggal'] = Carbon::parse($layanan['tanggal'])->format('j F Y'),
    ...
    public function layanan_excel(Request $request)
    {
        ...
        $data = Jadwal::leftjoin('layanan','jadwal.layanan_id','=','layanan.id')
                        ...
                        ->where('jadwal.tanggal','<=', $request->until = Carbon::parse($request->until))
                        ->where('status.nama','=','Selesai')
                        ->select('jadwal.tanggal as tanggal','a.name as psikolog','b.name as klien','b.jenis_kelamin as jenis_kelamin','kategori_klien.nama as kategori') 
                        ->orderBy('layanan.nama')
                        ->orderBy('tanggal','desc')
                        ->get()
                        ->toArray();
