...
class JadwalController extends Controller
{
    ...
    public function updateJadwal_psikolog(Request $request, $id){
        ...
        $data->save();
          ...
          if($request->status_id == 7){
           ...
           else if ($request->status_id == 12) {
             // konfirmasi pengalihan psikolog
             $optionBuilder = new OptionsBuilder();
             $optionBuilder->setTimeToLive(60*20);
           ...
           // dibatalkan
             ...
             $notificationBuilder = new PayloadNotificationBuilder('Konsultasi Telah Dibatalkan');
             $notificationBuilder->setBody('Konsultasi Telah Dibatalkan Oleh Klien')
                         ->setSound('default');
             $option = $optionBuilder->build();
             $notification = $notificationBuilder->build();
    ...
    public function index()
    {
      // if($request->status == 'all'){
        ...
        //   $list= Jadwal::leftjoin('status','jadwal.status_id','=','status.id')
        //               ->select('jadwal.tanggal', 'jadwal.id','jadwal.sesi_id','jadwal.klien_id','jadwal.ruangan_id','jadwal.psikolog_id', 'status.nama','jadwal.layanan_id','jadwal.keluhan')
        //               ->Where('status.nama as filter',$request->filter)
        //               ->orderBy('tanggal','desc')
        //               ->get();
        // }
    ...
    public function getSesi(Request $request)
    {
      ...
      if($check>6) {
        return response()->json(['Failed'=>"Data Sudah Tersedia!"]);
      } else if($check1>6) {
        return response()->json(['Failed'=>"Data Sudah Tersedia!"]);
      } else if($check2) {
        return response()->json(['Failed'=>"Data Sudah Tersedia!"]);
        ...
        return response()->json(['Success'=>"Lanjut!"]);
      } 
    }
